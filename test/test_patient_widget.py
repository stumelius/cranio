...
def test_patient_widget_updates_patients_from_database(database_patient_fixture):
    patient_widget = PatientWidget(database_patient_fixture)
    assert patient_widget.patient_count() == 1
    patient_widget.update_patients()
    assert patient_widget.patient_count() == 1
    with database_patient_fixture.session_scope() as s:
